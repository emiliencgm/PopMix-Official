...
def main():
    ...
    if world.config['augment'] in ['ED', 'RW']:
        augmentation = augments[world.config['augment']](world.config, Recmodel, precal)
    else:
        augmentation = None

    losss = {'BPR': loss.BPR_loss, 'BPR_Contrast':loss.BPR_Contrast_loss, 'BC':loss.BC_loss, 'Adaptive':loss.Adaptive_softmax_loss, 'PDA':loss.Causal_popularity_BPR_loss}
    ...
    try:
        ...
        for epoch in range(world.config['epochs']):
            ...
            avg_loss, avg_pop_acc = train.train(dataset, Recmodel, augmentation, epoch, optimizer, pop_class)
            end_train = time.time()
            ...
            with torch.no_grad():
                if epoch % 1== 0:
                    ...
                    if world.config['if_valid']:
                        ...
                        if result["recall"][0] > best_valid_recall:
                            stopping_valid_step = 0
                    ...
                    if result["recall"][0] > best_result_recall:#early stop
                        stopping_step = 0
